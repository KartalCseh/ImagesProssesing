...
def add_additive_noise(sigma_in):
    ...
    img_noise = cv2.add(img, noise, dtype=cv2.CV_8UC1)
...
def do_blur():
    ...
    img_noise_gauss_blur = cv2.GaussianBlur(img_noise, (gauss_blur_size, gauss_blur_size),
...
cv2.imshow('gauss blur', img_noise)
