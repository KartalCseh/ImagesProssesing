...
def mouse_event(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN and hsv_palette_circle_mask[y, x, 2] > 0:
        ...
        overlay = palette_BGR.copy()
        cv2.line(overlay, (HSV_CENTER_X, HSV_CENTER_Y), (HSV_CENTER_X + HSV_SIZE_HALF, HSV_CENTER_Y), (0, 0, 0), 1)
        cv2.line(overlay, (HSV_CENTER_X, HSV_CENTER_Y), (x, y), (255, 255, 255), 1)
        cv2.circle(overlay, (x, y), 3, (255, 255, 255), -1)
        cv2.imshow('HSV Circle', overlay)
        cv2.waitKey(5000)
        cv2.imshow('HSV Circle', palette_BGR)
